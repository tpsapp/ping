...
class GAME:
    def __init__(self, screen) -> None:
        ...
        self.paused = True
        self.beep_sound = pygame.mixer.Sound(settings.beep_sound)
        self.error_sound = pygame.mixer.Sound(settings.error_sound)
        self.win_sound = pygame.mixer.Sound(settings.win_sound)
        self.lose_sound = pygame.mixer.Sound(settings.lose_sound)
    ...
    def draw(self):
        ...
        if self.paused:
            pause_text = self.font.render(
                "Press Space to Continue", True, settings.white)
            pt_rect = pause_text.get_rect(
                center=(settings.screen_width / 2, settings.screen_height - settings.font_size))
    ...
    def draw_net(self):
        pygame.draw.aaline(self.screen, settings.blue, (settings.screen_width / 2,
                           0), (settings.screen_width / 2, settings.screen_height))
    ...
    def draw_score(self):
        player_score = self.font.render(
            str(self.player.score), True, settings.white)
        ps_rect = player_score.get_rect(
            center=(settings.screen_width * 0.75, settings.font_size))
        opponent_score = self.font.render(
            str(self.opponent.score), True, settings.white)
        os_rect = opponent_score.get_rect(
            center=(settings.screen_width * 0.25, settings.font_size))
