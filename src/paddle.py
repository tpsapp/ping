...
class PADDLE:
    ...
    def respawn(self):
        if self.side == 'left':
            self.pos = Vector2(0 + (self.width / 2) + 10, (settings.screen_height / 2) - (self.height / 2))
        else:
            self.pos = Vector2(settings.screen_width - (self.width / 2) - 20, (settings.screen_height / 2) - (self.height / 2))
        self.rect = pygame.rect.Rect(self.pos.x, self.pos.y, self.width, self.height)
        self.speed = 0
        self.score = 0
    ...
    def process_ai(self, ball_y):
        if self.rect.top < ball_y:
            print("top:", self.rect.top, ball_y)
            self.speed += 3
        if self.rect.bottom > ball_y:
            print("bottom:", self.rect.bottom, ball_y)
            self.speed -= 3
